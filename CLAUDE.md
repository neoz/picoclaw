...
- **Telegram reply context**: `handleMessage()` prepends `(replying to Name):\n> quoted text` (blockquote format) to content when the message replies to a non-bot user. `ReplyToMessage` is a full `*Message` with `Text`, `Caption`, `Photo`, `From`. Photos from the replied-to message are downloaded and added to `mediaPaths` (flowing through to `BuildMessages` as multimodal image parts). Skip replies to the bot itself (already in session history). Old stored logs with `[reply to X: Y]` format are normalized at display time by `normalizeReplyFormat()` in `stm.go` (pre-compiled regex `reOldReply`).
...
**run.sh commands**: `--build` (build image only, does not stop service), `--clean`, `--stop`, `--restart`, `--force` (build + clean + restart), `skills-list`, `skills-export`, `skills-import <dir>`, `memory-export`. Use `--restart --build` to rebuild and restart. The `memory-export` command copies the `workspace/memory/` folder (including `memory.db`) from the running container to `./memory-export/`.
